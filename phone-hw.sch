EESchema Schematic File Version 4
LIBS:phone-hw-cache
EELAYER 29 0
EELAYER END
$Descr USLetter 11000 8500
encoding utf-8
Sheet 1 5
Title ""
Date ""
Rev ""
Comp ""
Comment1 ""
Comment2 ""
Comment3 ""
Comment4 ""
$EndDescr
$Sheet
S 3000 800  1150 600 
U 5C8FFFB8
F0 "Battery protection" 50
F1 "battery_protection.sch" 50
F2 "VBAT_OUT" O R 4150 1050 50 
F3 "VBAT_IN" I L 3000 1050 50 
F4 "GND_OUT" O R 4150 1150 50 
F5 "GND_IN" I L 3000 1150 50 
$EndSheet
$Sheet
S 3000 1700 1150 600 
U 5C91848E
F0 "Battery charging" 50
F1 "battery_charging.sch" 50
F2 "VDD" I L 3000 1950 50 
F3 "VBAT" O R 4150 1850 50 
F4 "GND" I L 3000 2050 50 
F5 "VDDIO" O R 4150 1950 50 
F6 "~STDBY" O R 4150 2150 50 
F7 "~CHRG" O R 4150 2050 50 
$EndSheet
$Comp
L power:VBUS #PWR03
U 1 1 5C92835A
P 1650 1700
F 0 "#PWR03" H 1650 1550 50  0001 C CNN
F 1 "VBUS" H 1665 1873 50  0000 C CNN
F 2 "" H 1650 1700 50  0001 C CNN
F 3 "" H 1650 1700 50  0001 C CNN
	1    1650 1700
	1    0    0    -1  
$EndComp
Wire Wire Line
	1550 1800 1650 1800
Wire Wire Line
	1650 1800 1650 1700
$Comp
L power:GND #PWR02
U 1 1 5C928DD6
P 950 3400
F 0 "#PWR02" H 950 3150 50  0001 C CNN
F 1 "GND" H 955 3227 50  0000 C CNN
F 2 "" H 950 3400 50  0001 C CNN
F 3 "" H 950 3400 50  0001 C CNN
	1    950  3400
	1    0    0    -1  
$EndComp
Wire Wire Line
	950  3400 950  3350
Wire Wire Line
	950  3350 650  3350
Wire Wire Line
	650  3350 650  3300
Connection ~ 950  3350
Wire Wire Line
	950  3350 950  3300
Wire Wire Line
	1550 2500 1650 2500
Wire Wire Line
	1650 2500 1650 2550
Wire Wire Line
	1650 2600 1550 2600
Wire Wire Line
	1550 2300 1650 2300
Wire Wire Line
	1650 2300 1650 2350
Wire Wire Line
	1650 2400 1550 2400
$Comp
L Device:R_Small_US R3
U 1 1 5C92ACF5
P 2400 2200
F 0 "R3" H 2468 2246 50  0000 L CNN
F 1 "5.1k" H 2468 2155 50  0000 L CNN
F 2 "Resistor_SMD:R_0603_1608Metric_Pad1.05x0.95mm_HandSolder" H 2400 2200 50  0001 C CNN
F 3 "~" H 2400 2200 50  0001 C CNN
	1    2400 2200
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR06
U 1 1 5C92B516
P 2050 2550
F 0 "#PWR06" H 2050 2300 50  0001 C CNN
F 1 "GND" H 2055 2377 50  0000 C CNN
F 2 "" H 2050 2550 50  0001 C CNN
F 3 "" H 2050 2550 50  0001 C CNN
	1    2050 2550
	1    0    0    -1  
$EndComp
Wire Wire Line
	2050 2350 2050 2450
$Comp
L Device:R_Small_US R2
U 1 1 5C92A24A
P 2050 2250
F 0 "R2" H 2118 2296 50  0000 L CNN
F 1 "5.1k" H 2118 2205 50  0000 L CNN
F 2 "Resistor_SMD:R_0603_1608Metric_Pad1.05x0.95mm_HandSolder" H 2050 2250 50  0001 C CNN
F 3 "~" H 2050 2250 50  0001 C CNN
	1    2050 2250
	1    0    0    -1  
$EndComp
Wire Wire Line
	1550 2000 2400 2000
Wire Wire Line
	2400 2000 2400 2100
Wire Wire Line
	1550 2100 2050 2100
Wire Wire Line
	2050 2100 2050 2150
Wire Wire Line
	2400 2300 2400 2450
Wire Wire Line
	2400 2450 2050 2450
Connection ~ 2050 2450
Wire Wire Line
	2050 2450 2050 2550
$Comp
L Connector:USB_C_Receptacle_USB2.0 J1
U 1 1 5C9207EE
P 950 2400
F 0 "J1" H 1057 3267 50  0000 C CNN
F 1 "USB_C_Receptacle_USB2.0" H 1057 3176 50  0000 C CNN
F 2 "phone:U262-16XN-4BVC11" H 1100 2400 50  0001 C CNN
F 3 "https://www.usb.org/sites/default/files/documents/usb_type-c.zip" H 1100 2400 50  0001 C CNN
	1    950  2400
	1    0    0    -1  
$EndComp
NoConn ~ 1550 2900
NoConn ~ 1550 3000
Text GLabel 1700 2350 2    39   BiDi ~ 0
USB_D-
Text GLabel 1700 2550 2    39   BiDi ~ 0
USB_D+
Wire Wire Line
	1650 2550 1700 2550
Connection ~ 1650 2550
Wire Wire Line
	1650 2550 1650 2600
Wire Wire Line
	1700 2350 1650 2350
Connection ~ 1650 2350
Wire Wire Line
	1650 2350 1650 2400
$Comp
L power:VBUS #PWR08
U 1 1 5C92FEA9
P 2850 1850
F 0 "#PWR08" H 2850 1700 50  0001 C CNN
F 1 "VBUS" H 2865 2023 50  0000 C CNN
F 2 "" H 2850 1850 50  0001 C CNN
F 3 "" H 2850 1850 50  0001 C CNN
	1    2850 1850
	1    0    0    -1  
$EndComp
Wire Wire Line
	3000 1950 2850 1950
Wire Wire Line
	2850 1950 2850 1850
$Sheet
S 3000 2600 1150 600 
U 5C93115D
F0 "USB serial" 50
F1 "usb_serial.sch" 50
F2 "VBUS" I L 3000 2750 50 
F3 "VDDIO" I R 4150 2800 50 
F4 "GND" I L 3000 3050 50 
F5 "D+" B L 3000 2850 50 
F6 "D-" B L 3000 2950 50 
F7 "TX" O R 4150 2900 50 
F8 "RX" I R 4150 3000 50 
$EndSheet
Wire Wire Line
	3000 1050 2900 1050
Wire Wire Line
	2900 1050 2900 900 
Wire Wire Line
	2900 1300 2900 1150
Wire Wire Line
	2900 1150 3000 1150
$Comp
L power:GND #PWR019
U 1 1 5C939262
P 4300 1250
F 0 "#PWR019" H 4300 1000 50  0001 C CNN
F 1 "GND" H 4305 1077 50  0000 C CNN
F 2 "" H 4300 1250 50  0001 C CNN
F 3 "" H 4300 1250 50  0001 C CNN
	1    4300 1250
	1    0    0    -1  
$EndComp
Wire Wire Line
	4150 1150 4300 1150
Wire Wire Line
	4300 1150 4300 1250
$Comp
L power:+BATT #PWR018
U 1 1 5C93ACD9
P 4300 950
F 0 "#PWR018" H 4300 800 50  0001 C CNN
F 1 "+BATT" H 4315 1123 50  0000 C CNN
F 2 "" H 4300 950 50  0001 C CNN
F 3 "" H 4300 950 50  0001 C CNN
	1    4300 950 
	1    0    0    -1  
$EndComp
Wire Wire Line
	4150 1050 4300 1050
Wire Wire Line
	4300 1050 4300 950 
$Comp
L power:GND #PWR09
U 1 1 5C93BAEC
P 2850 2150
F 0 "#PWR09" H 2850 1900 50  0001 C CNN
F 1 "GND" H 2855 1977 50  0000 C CNN
F 2 "" H 2850 2150 50  0001 C CNN
F 3 "" H 2850 2150 50  0001 C CNN
	1    2850 2150
	1    0    0    -1  
$EndComp
Wire Wire Line
	3000 2050 2850 2050
Wire Wire Line
	2850 2050 2850 2150
$Comp
L power:+BATT #PWR020
U 1 1 5C93D731
P 4300 1800
F 0 "#PWR020" H 4300 1650 50  0001 C CNN
F 1 "+BATT" H 4315 1973 50  0000 C CNN
F 2 "" H 4300 1800 50  0001 C CNN
F 3 "" H 4300 1800 50  0001 C CNN
	1    4300 1800
	1    0    0    -1  
$EndComp
Wire Wire Line
	4150 1850 4300 1850
Wire Wire Line
	4300 1850 4300 1800
Text GLabel 4300 2050 2    39   Output ~ 0
BATT_~CHRG
Wire Wire Line
	4150 2050 4300 2050
$Comp
L power:GND #PWR011
U 1 1 5C947E22
P 2850 3100
F 0 "#PWR011" H 2850 2850 50  0001 C CNN
F 1 "GND" H 2855 2927 50  0000 C CNN
F 2 "" H 2850 3100 50  0001 C CNN
F 3 "" H 2850 3100 50  0001 C CNN
	1    2850 3100
	1    0    0    -1  
$EndComp
Wire Wire Line
	3000 3050 2850 3050
Wire Wire Line
	2850 3050 2850 3100
$Comp
L power:VBUS #PWR010
U 1 1 5C948A02
P 2850 2700
F 0 "#PWR010" H 2850 2550 50  0001 C CNN
F 1 "VBUS" H 2865 2873 50  0000 C CNN
F 2 "" H 2850 2700 50  0001 C CNN
F 3 "" H 2850 2700 50  0001 C CNN
	1    2850 2700
	1    0    0    -1  
$EndComp
Wire Wire Line
	3000 2750 2850 2750
Wire Wire Line
	2850 2750 2850 2700
Text GLabel 2950 2950 0    39   BiDi ~ 0
USB_D-
Text GLabel 2950 2850 0    39   BiDi ~ 0
USB_D+
Wire Wire Line
	2950 2850 3000 2850
Wire Wire Line
	3000 2950 2950 2950
$Comp
L power:VDD #PWR021
U 1 1 5C94ACAC
P 4300 2750
F 0 "#PWR021" H 4300 2600 50  0001 C CNN
F 1 "VDD" H 4317 2923 50  0000 C CNN
F 2 "" H 4300 2750 50  0001 C CNN
F 3 "" H 4300 2750 50  0001 C CNN
	1    4300 2750
	1    0    0    -1  
$EndComp
Wire Wire Line
	4300 2750 4300 2800
Wire Wire Line
	4300 2800 4150 2800
Text GLabel 4300 2900 2    39   Output ~ 0
UART0_RX
Text GLabel 4300 3000 2    39   Input ~ 0
UART0_TX
Wire Wire Line
	4150 3000 4300 3000
Wire Wire Line
	4300 2900 4150 2900
$Comp
L phone:XC6222B281MR-G U3
U 1 1 5C9500CF
P 5750 1100
F 0 "U3" H 5750 1467 50  0000 C CNN
F 1 "XC6222B281MR-G" H 5750 1376 50  0000 C CNN
F 2 "Package_TO_SOT_SMD:SOT-23-5" H 5750 1100 50  0001 C CNN
F 3 "https://www.torexsemi.com/file/xc6222/XC6222.pdf" H 6500 100 50  0001 C CNN
	1    5750 1100
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR031
U 1 1 5C950FA9
P 5750 1600
F 0 "#PWR031" H 5750 1350 50  0001 C CNN
F 1 "GND" H 5755 1427 50  0000 C CNN
F 2 "" H 5750 1600 50  0001 C CNN
F 3 "" H 5750 1600 50  0001 C CNN
	1    5750 1600
	1    0    0    -1  
$EndComp
Wire Wire Line
	5750 1600 5750 1550
$Comp
L power:VDD #PWR034
U 1 1 5C951ECB
P 6300 950
F 0 "#PWR034" H 6300 800 50  0001 C CNN
F 1 "VDD" H 6317 1123 50  0000 C CNN
F 2 "" H 6300 950 50  0001 C CNN
F 3 "" H 6300 950 50  0001 C CNN
	1    6300 950 
	1    0    0    -1  
$EndComp
Wire Wire Line
	6300 950  6300 1000
Wire Wire Line
	6300 1000 6250 1000
$Comp
L power:+BATT #PWR027
U 1 1 5C9535D2
P 5200 950
F 0 "#PWR027" H 5200 800 50  0001 C CNN
F 1 "+BATT" H 5215 1123 50  0000 C CNN
F 2 "" H 5200 950 50  0001 C CNN
F 3 "" H 5200 950 50  0001 C CNN
	1    5200 950 
	1    0    0    -1  
$EndComp
Wire Wire Line
	5250 1000 5200 1000
Wire Wire Line
	5200 1000 5200 950 
Wire Wire Line
	5200 1000 5200 1200
Wire Wire Line
	5200 1200 5250 1200
Connection ~ 5200 1000
$Comp
L Device:C_Small C3
U 1 1 5C9559D7
P 5200 1400
F 0 "C3" H 5292 1446 50  0000 L CNN
F 1 "1uF" H 5292 1355 50  0000 L CNN
F 2 "Capacitor_SMD:C_0603_1608Metric_Pad1.05x0.95mm_HandSolder" H 5200 1400 50  0001 C CNN
F 3 "~" H 5200 1400 50  0001 C CNN
	1    5200 1400
	1    0    0    -1  
$EndComp
Wire Wire Line
	5200 1300 5200 1200
Connection ~ 5200 1200
Wire Wire Line
	5200 1500 5200 1550
Wire Wire Line
	5200 1550 5750 1550
Connection ~ 5750 1550
Wire Wire Line
	5750 1550 5750 1500
$Comp
L Device:C_Small C6
U 1 1 5C9598CE
P 6300 1400
F 0 "C6" H 6392 1446 50  0000 L CNN
F 1 "4.7uF" H 6392 1355 50  0000 L CNN
F 2 "Capacitor_SMD:C_0603_1608Metric_Pad1.05x0.95mm_HandSolder" H 6300 1400 50  0001 C CNN
F 3 "~" H 6300 1400 50  0001 C CNN
	1    6300 1400
	1    0    0    -1  
$EndComp
Wire Wire Line
	5750 1550 6300 1550
Wire Wire Line
	6300 1550 6300 1500
Wire Wire Line
	6300 1300 6300 1250
Connection ~ 6300 1000
$Comp
L power:VDD #PWR04
U 1 1 5C986962
P 1850 3900
F 0 "#PWR04" H 1850 3750 50  0001 C CNN
F 1 "VDD" H 1867 4073 50  0000 C CNN
F 2 "" H 1850 3900 50  0001 C CNN
F 3 "" H 1850 3900 50  0001 C CNN
	1    1850 3900
	1    0    0    -1  
$EndComp
Wire Wire Line
	1850 4400 1850 3950
Text GLabel 2550 4900 2    39   Input ~ 0
UART0_RX
Wire Wire Line
	2550 4900 2450 4900
Text GLabel 2550 4700 2    39   Output ~ 0
UART0_TX
Wire Wire Line
	2450 4700 2550 4700
Wire Wire Line
	1250 4600 1150 4600
Wire Wire Line
	1150 3950 1850 3950
Connection ~ 1850 3950
Wire Wire Line
	1850 3950 1850 3900
NoConn ~ 1250 5800
NoConn ~ 1250 5900
NoConn ~ 1250 6000
NoConn ~ 1250 6100
NoConn ~ 1250 6200
NoConn ~ 1250 6300
Text GLabel 2550 5500 2    39   Output ~ 0
JTAG_MTDO
Text GLabel 2550 5300 2    39   Input ~ 0
JTAG_MTCK
Text GLabel 2550 5400 2    39   Input ~ 0
JTAG_MTMS
Text GLabel 2550 5200 2    39   Input ~ 0
JTAG_MTDI
Wire Wire Line
	2450 5200 2550 5200
Wire Wire Line
	2550 5300 2450 5300
Wire Wire Line
	2450 5400 2550 5400
Wire Wire Line
	2550 5500 2450 5500
$Comp
L power:GND #PWR05
U 1 1 5C99F091
P 1850 7250
F 0 "#PWR05" H 1850 7000 50  0001 C CNN
F 1 "GND" H 1855 7077 50  0000 C CNN
F 2 "" H 1850 7250 50  0001 C CNN
F 3 "" H 1850 7250 50  0001 C CNN
	1    1850 7250
	1    0    0    -1  
$EndComp
Wire Wire Line
	1850 7250 1850 7200
$Comp
L Device:R_Small_US R1
U 1 1 5C9A2ECC
P 1150 4100
F 0 "R1" H 1218 4146 50  0000 L CNN
F 1 "10k" H 1218 4055 50  0000 L CNN
F 2 "Resistor_SMD:R_0603_1608Metric_Pad1.05x0.95mm_HandSolder" H 1150 4100 50  0001 C CNN
F 3 "~" H 1150 4100 50  0001 C CNN
	1    1150 4100
	1    0    0    -1  
$EndComp
Wire Wire Line
	1150 4200 1150 4250
Wire Wire Line
	1150 4000 1150 3950
Wire Wire Line
	650  4250 600  4250
$Comp
L power:GND #PWR01
U 1 1 5C9B889C
P 600 4300
F 0 "#PWR01" H 600 4050 50  0001 C CNN
F 1 "GND" H 605 4127 50  0000 C CNN
F 2 "" H 600 4300 50  0001 C CNN
F 3 "" H 600 4300 50  0001 C CNN
	1    600  4300
	1    0    0    -1  
$EndComp
$Comp
L Device:C_Small C1
U 1 1 5C9BB39E
P 2200 4100
F 0 "C1" H 2292 4146 50  0000 L CNN
F 1 "22uF" H 2292 4055 50  0000 L CNN
F 2 "Capacitor_SMD:C_0603_1608Metric_Pad1.05x0.95mm_HandSolder" H 2200 4100 50  0001 C CNN
F 3 "~" H 2200 4100 50  0001 C CNN
	1    2200 4100
	1    0    0    -1  
$EndComp
$Comp
L Device:C_Small C2
U 1 1 5C9BB63E
P 2600 4100
F 0 "C2" H 2692 4146 50  0000 L CNN
F 1 "0.1uF" H 2692 4055 50  0000 L CNN
F 2 "Capacitor_SMD:C_0603_1608Metric_Pad1.05x0.95mm_HandSolder" H 2600 4100 50  0001 C CNN
F 3 "~" H 2600 4100 50  0001 C CNN
	1    2600 4100
	1    0    0    -1  
$EndComp
Wire Wire Line
	1850 3950 2200 3950
Wire Wire Line
	2200 3950 2200 4000
Wire Wire Line
	2200 3950 2600 3950
Wire Wire Line
	2600 3950 2600 4000
Connection ~ 2200 3950
$Comp
L power:GND #PWR07
U 1 1 5C9C48FE
P 2600 4300
F 0 "#PWR07" H 2600 4050 50  0001 C CNN
F 1 "GND" H 2605 4127 50  0000 C CNN
F 2 "" H 2600 4300 50  0001 C CNN
F 3 "" H 2600 4300 50  0001 C CNN
	1    2600 4300
	1    0    0    -1  
$EndComp
Wire Wire Line
	2200 4200 2200 4250
Wire Wire Line
	2200 4250 2600 4250
Wire Wire Line
	2600 4250 2600 4200
$Comp
L Switch:SW_Push SW2
U 1 1 5C9DD1E7
P 2900 4600
F 0 "SW2" H 2900 4925 50  0000 C CNN
F 1 "SW_Push" H 2900 4834 50  0000 C CNN
F 2 "Button_Switch_SMD:SW_SPST_EVQPE1" H 2900 4600 50  0001 C CNN
F 3 "~" H 2900 4600 50  0001 C CNN
	1    2900 4600
	1    0    0    -1  
$EndComp
Wire Wire Line
	2600 4300 2600 4250
Connection ~ 2600 4250
Wire Wire Line
	2450 4600 2700 4600
Wire Wire Line
	3100 4600 3150 4600
$Comp
L power:GND #PWR013
U 1 1 5CA21102
P 3150 4650
F 0 "#PWR013" H 3150 4400 50  0001 C CNN
F 1 "GND" H 3155 4477 50  0000 C CNN
F 2 "" H 3150 4650 50  0001 C CNN
F 3 "" H 3150 4650 50  0001 C CNN
	1    3150 4650
	1    0    0    -1  
$EndComp
$Comp
L phone:L80-R U6
U 1 1 5CA2E2F4
P 9050 2650
F 0 "U6" H 8457 2688 39  0000 R CNN
F 1 "L80-R" H 8457 2613 39  0000 R CNN
F 2 "phone:L80-R" H 9050 2650 39  0001 C CNN
F 3 "https://www.quectel.com/UploadImage/Downlad/Quectel_L80-R_Hardware_Design_V1.2.pdf" H 9050 2650 39  0001 C CNN
	1    9050 2650
	1    0    0    -1  
$EndComp
$Comp
L power:+BATT #PWR048
U 1 1 5CA42573
P 9000 1750
F 0 "#PWR048" H 9000 1600 50  0001 C CNN
F 1 "+BATT" H 9015 1923 50  0000 C CNN
F 2 "" H 9000 1750 50  0001 C CNN
F 3 "" H 9000 1750 50  0001 C CNN
	1    9000 1750
	1    0    0    -1  
$EndComp
Wire Wire Line
	9000 2100 9000 2050
Wire Wire Line
	9100 2100 9100 2050
Wire Wire Line
	9100 2050 9000 2050
Connection ~ 9000 2050
Wire Wire Line
	9000 2050 9000 1800
$Comp
L power:GND #PWR049
U 1 1 5CA4F3D7
P 9050 3300
F 0 "#PWR049" H 9050 3050 50  0001 C CNN
F 1 "GND" H 9055 3127 50  0000 C CNN
F 2 "" H 9050 3300 50  0001 C CNN
F 3 "" H 9050 3300 50  0001 C CNN
	1    9050 3300
	1    0    0    -1  
$EndComp
Wire Wire Line
	9000 3200 9000 3250
Wire Wire Line
	9000 3250 9050 3250
Wire Wire Line
	9100 3250 9100 3200
Wire Wire Line
	9050 3300 9050 3250
Connection ~ 9050 3250
Wire Wire Line
	9050 3250 9100 3250
$Comp
L Device:C_Small C10
U 1 1 5CA59CF4
P 9400 1950
F 0 "C10" H 9492 1996 50  0000 L CNN
F 1 "10uF" H 9492 1905 50  0000 L CNN
F 2 "Capacitor_SMD:C_0603_1608Metric_Pad1.05x0.95mm_HandSolder" H 9400 1950 50  0001 C CNN
F 3 "~" H 9400 1950 50  0001 C CNN
	1    9400 1950
	1    0    0    -1  
$EndComp
$Comp
L Device:C_Small C11
U 1 1 5CA5A0A8
P 9800 1950
F 0 "C11" H 9892 1996 50  0000 L CNN
F 1 "0.1uF" H 9892 1905 50  0000 L CNN
F 2 "Capacitor_SMD:C_0603_1608Metric_Pad1.05x0.95mm_HandSolder" H 9800 1950 50  0001 C CNN
F 3 "~" H 9800 1950 50  0001 C CNN
	1    9800 1950
	1    0    0    -1  
$EndComp
$Comp
L Device:C_Small C12
U 1 1 5CA5A8F5
P 10200 1950
F 0 "C12" H 10292 1996 50  0000 L CNN
F 1 "0.1uF" H 10292 1905 50  0000 L CNN
F 2 "Capacitor_SMD:C_0603_1608Metric_Pad1.05x0.95mm_HandSolder" H 10200 1950 50  0001 C CNN
F 3 "~" H 10200 1950 50  0001 C CNN
	1    10200 1950
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR052
U 1 1 5CA5B8F7
P 9800 2150
F 0 "#PWR052" H 9800 1900 50  0001 C CNN
F 1 "GND" H 9805 1977 50  0000 C CNN
F 2 "" H 9800 2150 50  0001 C CNN
F 3 "" H 9800 2150 50  0001 C CNN
	1    9800 2150
	1    0    0    -1  
$EndComp
Wire Wire Line
	9000 1800 9400 1800
Wire Wire Line
	9400 1800 9400 1850
Connection ~ 9000 1800
Wire Wire Line
	9000 1800 9000 1750
Wire Wire Line
	9400 1800 9800 1800
Wire Wire Line
	9800 1800 9800 1850
Connection ~ 9400 1800
Wire Wire Line
	9800 1800 10200 1800
Wire Wire Line
	10200 1800 10200 1850
Connection ~ 9800 1800
Wire Wire Line
	9800 2150 9800 2100
Wire Wire Line
	9400 2050 9400 2100
Wire Wire Line
	9400 2100 9800 2100
Connection ~ 9800 2100
Wire Wire Line
	9800 2100 9800 2050
Wire Wire Line
	9800 2100 10200 2100
Wire Wire Line
	10200 2100 10200 2050
Text GLabel 9700 2550 2    39   Output ~ 0
GPS_TXD
Text GLabel 9700 2650 2    39   Input ~ 0
GPS_RXD
Wire Wire Line
	9600 2650 9700 2650
Wire Wire Line
	9700 2550 9600 2550
Text GLabel 8200 2650 0    39   Output ~ 0
GPS_RESET
Wire Wire Line
	8200 2650 8350 2650
$Comp
L Device:R_Small_US R13
U 1 1 5CA89067
P 8350 2450
F 0 "R13" H 8282 2404 50  0000 R CNN
F 1 "10k" H 8282 2495 50  0000 R CNN
F 2 "Resistor_SMD:R_0603_1608Metric_Pad1.05x0.95mm_HandSolder" H 8350 2450 50  0001 C CNN
F 3 "~" H 8350 2450 50  0001 C CNN
	1    8350 2450
	1    0    0    1   
$EndComp
Wire Wire Line
	8350 2550 8350 2650
Connection ~ 8350 2650
Wire Wire Line
	8350 2650 8500 2650
$Sheet
S 8450 800  1150 600 
U 5C9AE52D
F0 "Cellular" 50
F1 "cellular.sch" 50
F2 "RXD" I R 9600 1050 50 
F3 "TXD" O R 9600 1150 50 
F4 "TXD_AUX" O L 8450 1000 50 
F5 "DBG_TXD" O L 8450 1250 50 
F6 "RXD_AUX" I L 8450 900 50 
F7 "DBG_RXD" I L 8450 1150 50 
F8 "PWRKEY" I R 9600 900 50 
F9 "RI" O R 9600 1300 50 
$EndSheet
$Comp
L phone:ER-TFT1.44-1 U5
U 1 1 5C9C3706
P 8350 5650
F 0 "U5" H 8350 6578 50  0000 C CNN
F 1 "ER-TFT1.44-1" H 8350 6487 50  0000 C CNN
F 2 "phone:ER-TFT1.44-1-FOLD" H 8350 5650 50  0001 C CNN
F 3 "" H 8350 5650 50  0001 C CNN
	1    8350 5650
	1    0    0    -1  
$EndComp
Text GLabel 7850 5500 0    39   BiDi ~ 0
LCD_SDA
Wire Wire Line
	7850 5500 7900 5500
Text GLabel 7850 5400 0    39   Input ~ 0
LCD_SCL
Wire Wire Line
	7850 5400 7900 5400
Text GLabel 7850 5300 0    39   Input ~ 0
LCD_~CS
Text GLabel 7850 5600 0    39   Input ~ 0
LCD_DC
Wire Wire Line
	7850 5600 7900 5600
Text GLabel 7100 5750 0    39   Input ~ 0
LCD_RESET
Wire Wire Line
	7100 5750 7250 5750
$Comp
L Device:R_Small_US R11
U 1 1 5CA0B9D8
P 7250 5550
F 0 "R11" H 7182 5504 50  0000 R CNN
F 1 "10k" H 7182 5595 50  0000 R CNN
F 2 "Resistor_SMD:R_0603_1608Metric_Pad1.05x0.95mm_HandSolder" H 7250 5550 50  0001 C CNN
F 3 "~" H 7250 5550 50  0001 C CNN
	1    7250 5550
	1    0    0    1   
$EndComp
Wire Wire Line
	7250 5750 7250 5650
Connection ~ 7250 5750
Wire Wire Line
	7250 5750 7900 5750
$Comp
L power:VDD #PWR045
U 1 1 5CA14D24
P 8300 4350
F 0 "#PWR045" H 8300 4200 50  0001 C CNN
F 1 "VDD" H 8317 4523 50  0000 C CNN
F 2 "" H 8300 4350 50  0001 C CNN
F 3 "" H 8300 4350 50  0001 C CNN
	1    8300 4350
	1    0    0    -1  
$EndComp
Wire Wire Line
	8300 4350 8300 4400
Wire Wire Line
	8300 4400 8300 4900
$Comp
L power:VDD #PWR039
U 1 1 5CA22A5E
P 7250 5350
F 0 "#PWR039" H 7250 5200 50  0001 C CNN
F 1 "VDD" H 7267 5523 50  0000 C CNN
F 2 "" H 7250 5350 50  0001 C CNN
F 3 "" H 7250 5350 50  0001 C CNN
	1    7250 5350
	1    0    0    -1  
$EndComp
Wire Wire Line
	7250 5350 7250 5450
Wire Wire Line
	8400 6400 8400 6450
Wire Wire Line
	8300 6450 8300 6400
$Comp
L power:GND #PWR042
U 1 1 5CA36BA3
P 7850 6050
F 0 "#PWR042" H 7850 5800 50  0001 C CNN
F 1 "GND" H 7855 5877 50  0000 C CNN
F 2 "" H 7850 6050 50  0001 C CNN
F 3 "" H 7850 6050 50  0001 C CNN
	1    7850 6050
	1    0    0    -1  
$EndComp
Wire Wire Line
	7900 6000 7850 6000
Wire Wire Line
	7850 6000 7850 6050
$Comp
L power:VDD #PWR035
U 1 1 5CA3BE48
P 6650 5800
F 0 "#PWR035" H 6650 5650 50  0001 C CNN
F 1 "VDD" H 6667 5973 50  0000 C CNN
F 2 "" H 6650 5800 50  0001 C CNN
F 3 "" H 6650 5800 50  0001 C CNN
	1    6650 5800
	1    0    0    -1  
$EndComp
Wire Wire Line
	7900 5900 6650 5900
Wire Wire Line
	6650 5900 6650 5800
$Comp
L Device:C_Small C9
U 1 1 5CA51B79
P 7950 4550
F 0 "C9" H 8042 4596 50  0000 L CNN
F 1 "0.1uF" H 8042 4505 50  0000 L CNN
F 2 "Capacitor_SMD:C_0603_1608Metric_Pad1.05x0.95mm_HandSolder" H 7950 4550 50  0001 C CNN
F 3 "~" H 7950 4550 50  0001 C CNN
	1    7950 4550
	1    0    0    -1  
$EndComp
Text GLabel 8850 5100 2    39   Output ~ 0
LCD_TE
Wire Wire Line
	8800 5100 8850 5100
Wire Wire Line
	8800 6200 8850 6200
Wire Wire Line
	8850 6200 8850 6100
Wire Wire Line
	8800 5600 8850 5600
Connection ~ 8850 5600
Wire Wire Line
	8850 5600 8850 5500
Wire Wire Line
	8800 5700 8850 5700
Connection ~ 8850 5700
Wire Wire Line
	8850 5700 8850 5600
Wire Wire Line
	8800 5800 8850 5800
Connection ~ 8850 5800
Wire Wire Line
	8850 5800 8850 5700
Wire Wire Line
	8800 5900 8850 5900
Connection ~ 8850 5900
Wire Wire Line
	8850 5900 8850 5800
Wire Wire Line
	8800 6000 8850 6000
Connection ~ 8850 6000
Wire Wire Line
	8850 6000 8850 5900
Wire Wire Line
	8800 6100 8850 6100
Connection ~ 8850 6100
Wire Wire Line
	8850 6100 8850 6000
Wire Wire Line
	8850 6450 8400 6450
Wire Wire Line
	8300 6450 8400 6450
Connection ~ 8400 6450
Wire Wire Line
	8850 6500 8850 6450
Wire Wire Line
	7850 5300 7900 5300
Wire Wire Line
	8800 5350 9050 5350
$Comp
L phone:NVT2008 U4
U 1 1 5C9DBE6C
P 6250 3650
F 0 "U4" H 6250 4415 50  0000 C CNN
F 1 "NVT2008" H 6250 4324 50  0000 C CNN
F 2 "Package_SO:TSSOP-20_4.4x6.5mm_P0.65mm" H 6250 3650 50  0001 C CNN
F 3 "" H 6250 3650 50  0001 C CNN
	1    6250 3650
	1    0    0    -1  
$EndComp
$Comp
L power:VDD #PWR032
U 1 1 5C9E98AE
P 5800 2600
F 0 "#PWR032" H 5800 2450 50  0001 C CNN
F 1 "VDD" H 5817 2773 50  0000 C CNN
F 2 "" H 5800 2600 50  0001 C CNN
F 3 "" H 5800 2600 50  0001 C CNN
	1    5800 2600
	1    0    0    -1  
$EndComp
Wire Wire Line
	5850 3150 5800 3150
Wire Wire Line
	5800 3150 5800 2650
$Comp
L power:+BATT #PWR036
U 1 1 5C9F06C1
P 6700 2600
F 0 "#PWR036" H 6700 2450 50  0001 C CNN
F 1 "+BATT" H 6715 2773 50  0000 C CNN
F 2 "" H 6700 2600 50  0001 C CNN
F 3 "" H 6700 2600 50  0001 C CNN
	1    6700 2600
	1    0    0    -1  
$EndComp
Wire Wire Line
	6650 3150 6700 3150
Wire Wire Line
	6700 3150 6700 2650
Wire Wire Line
	6650 3250 6700 3250
Wire Wire Line
	6700 3250 6700 3150
Connection ~ 6700 3150
$Comp
L power:GND #PWR033
U 1 1 5CA0C7FD
P 6250 4350
F 0 "#PWR033" H 6250 4100 50  0001 C CNN
F 1 "GND" H 6255 4177 50  0000 C CNN
F 2 "" H 6250 4350 50  0001 C CNN
F 3 "" H 6250 4350 50  0001 C CNN
	1    6250 4350
	1    0    0    -1  
$EndComp
Wire Wire Line
	6250 4350 6250 4300
Text GLabel 5450 3500 0    39   Input ~ 0
UART1_TX
Text GLabel 6700 3400 2    39   Input ~ 0
GPS_TXD
Text GLabel 7200 3500 2    39   Output ~ 0
GPS_RXD
Wire Wire Line
	6650 3500 7100 3500
Wire Wire Line
	6650 3400 6700 3400
Wire Wire Line
	5850 3400 5250 3400
Wire Wire Line
	5450 3500 5850 3500
Text GLabel 2550 6800 2    39   Input ~ 0
UART1_RX
Text GLabel 2550 5100 2    39   Output ~ 0
UART1_TX
Text GLabel 9700 900  2    39   Output ~ 0
GSM_PWRKEY
Text GLabel 9700 1050 2    39   Input ~ 0
GSM_RXD
Text GLabel 9700 1150 2    39   Output ~ 0
GSM_TXD
Wire Wire Line
	9600 900  9700 900 
Wire Wire Line
	9700 1050 9600 1050
Wire Wire Line
	9700 1150 9600 1150
Text GLabel 7200 3700 2    39   Output ~ 0
GSM_PWRKEY
Text GLabel 2550 6100 2    39   BiDi ~ 0
LCD_SDA
Text GLabel 2550 6000 2    39   Output ~ 0
LCD_SCL
Text GLabel 2550 6200 2    39   Output ~ 0
LCD_DC
Text GLabel 7200 3600 2    39   Output ~ 0
GPS_RESET
Wire Wire Line
	6650 3700 7100 3700
$Comp
L power:+BATT #PWR046
U 1 1 5CAC81E9
P 8350 2250
F 0 "#PWR046" H 8350 2100 50  0001 C CNN
F 1 "+BATT" H 8365 2423 50  0000 C CNN
F 2 "" H 8350 2250 50  0001 C CNN
F 3 "" H 8350 2250 50  0001 C CNN
	1    8350 2250
	1    0    0    -1  
$EndComp
Wire Wire Line
	8350 2250 8350 2350
Text GLabel 7650 3900 2    39   Output ~ 0
GSM_RXD
Text GLabel 6700 3800 2    39   Input ~ 0
GSM_TXD
Wire Wire Line
	6650 3800 6700 3800
Wire Wire Line
	6650 3900 7450 3900
$Comp
L phone:TSWB-3N-CB SW?
U 1 1 5CAF4DA9
P 3550 7350
AR Path="/5C9D7881/5CAF4DA9" Ref="SW?"  Part="1" 
AR Path="/5CAF4DA9" Ref="SW3"  Part="1" 
F 0 "SW3" H 3550 7915 50  0000 C CNN
F 1 "TSWB-3N-CB" H 3550 7824 50  0000 C CNN
F 2 "phone:TSWB-3N-CB" H 3550 7350 50  0001 C CNN
F 3 "" H 3550 7350 50  0001 C CNN
	1    3550 7350
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR?
U 1 1 5CAF4DB0
P 3150 7450
AR Path="/5C9D7881/5CAF4DB0" Ref="#PWR?"  Part="1" 
AR Path="/5CAF4DB0" Ref="#PWR014"  Part="1" 
F 0 "#PWR014" H 3150 7200 50  0001 C CNN
F 1 "GND" H 3155 7277 50  0000 C CNN
F 2 "" H 3150 7450 50  0001 C CNN
F 3 "" H 3150 7450 50  0001 C CNN
	1    3150 7450
	1    0    0    -1  
$EndComp
Wire Wire Line
	3200 7300 3150 7300
Wire Wire Line
	3150 7300 3150 7400
Wire Wire Line
	3150 7400 3200 7400
Connection ~ 3150 7400
Wire Wire Line
	3150 7400 3150 7450
Wire Wire Line
	3900 7150 3950 7150
Wire Wire Line
	3950 7250 3900 7250
Wire Wire Line
	3900 7350 3950 7350
Wire Wire Line
	3950 7450 3900 7450
Wire Wire Line
	3900 7550 3950 7550
Wire Wire Line
	3950 7650 3900 7650
Text GLabel 3950 7050 2    39   Output ~ 0
INPUT_S1
Wire Wire Line
	3900 7050 3950 7050
Text GLabel 3950 7150 2    39   Output ~ 0
INPUT_S2
Text GLabel 3950 7250 2    39   Output ~ 0
INPUT_S3
Text GLabel 3950 7350 2    39   Output ~ 0
INPUT_S4
Text GLabel 3950 7450 2    39   Output ~ 0
INPUT_S5
Text GLabel 3950 7550 2    39   Output ~ 0
INPUT_A
Text GLabel 3950 7650 2    39   Output ~ 0
INPUT_B
Text GLabel 5550 5450 2    39   Input ~ 0
INPUT_S1
Text GLabel 5550 5550 2    39   Input ~ 0
INPUT_S2
Text GLabel 5550 5650 2    39   Input ~ 0
INPUT_S3
Text GLabel 5550 5750 2    39   Input ~ 0
INPUT_S4
Text GLabel 5550 5850 2    39   Input ~ 0
INPUT_S5
Text GLabel 5550 5950 2    39   Input ~ 0
INPUT_A
Text GLabel 5550 6050 2    39   Input ~ 0
INPUT_B
Text GLabel 4750 3800 0    39   Output ~ 0
UART2_RX
Text GLabel 5450 3900 0    39   Input ~ 0
UART2_TX
Wire Wire Line
	5450 3900 5850 3900
Text GLabel 2550 6900 2    39   Input ~ 0
UART2_RX
Text GLabel 2550 5800 2    39   Output ~ 0
UART2_TX
Connection ~ 1150 4250
Wire Wire Line
	1150 4250 1150 4600
$Comp
L phone:ESP32-WROVER U1
U 1 1 5C96ECAB
P 1850 5800
F 0 "U1" H 1400 7150 50  0000 C CNN
F 1 "ESP32-WROVER" H 2200 7150 50  0000 C CNN
F 2 "phone:ESP32-WROVER" H 1850 4300 50  0001 C CNN
F 3 "https://www.espressif.com/sites/default/files/documentation/esp32-wroom-32_datasheet_en.pdf" H 1550 5850 50  0001 C CNN
	1    1850 5800
	1    0    0    -1  
$EndComp
Wire Wire Line
	2450 6800 2550 6800
Wire Wire Line
	2550 6900 2450 6900
Wire Wire Line
	2450 6700 2550 6700
Wire Wire Line
	2450 6600 2550 6600
Wire Wire Line
	2450 6500 2550 6500
Wire Wire Line
	3350 6400 3250 6400
Wire Wire Line
	2450 6300 2950 6300
Wire Wire Line
	2450 6200 2550 6200
Wire Wire Line
	2450 6100 2550 6100
Wire Wire Line
	2450 6000 2550 6000
Wire Wire Line
	2450 5900 2550 5900
Wire Wire Line
	2450 5800 2550 5800
Wire Wire Line
	2450 5100 2550 5100
Wire Wire Line
	2450 5000 2550 5000
Wire Wire Line
	2450 4800 2550 4800
Wire Wire Line
	4750 3800 4800 3800
Wire Wire Line
	5450 3600 5850 3600
Wire Wire Line
	5450 3700 5850 3700
Text GLabel 5200 3400 0    39   Output ~ 0
UART1_RX
$Comp
L Device:R_Small_US R8
U 1 1 5CD3579D
P 5250 3250
F 0 "R8" H 5318 3296 50  0000 L CNN
F 1 "10k" H 5318 3205 50  0000 L CNN
F 2 "Resistor_SMD:R_0603_1608Metric_Pad1.05x0.95mm_HandSolder" H 5250 3250 50  0001 C CNN
F 3 "~" H 5250 3250 50  0001 C CNN
	1    5250 3250
	1    0    0    -1  
$EndComp
Wire Wire Line
	5250 3400 5250 3350
Connection ~ 5250 3400
Wire Wire Line
	5250 3400 5200 3400
$Comp
L power:VDD #PWR028
U 1 1 5CD621CF
P 5250 3100
F 0 "#PWR028" H 5250 2950 50  0001 C CNN
F 1 "VDD" H 5267 3273 50  0000 C CNN
F 2 "" H 5250 3100 50  0001 C CNN
F 3 "" H 5250 3100 50  0001 C CNN
	1    5250 3100
	1    0    0    -1  
$EndComp
Wire Wire Line
	5250 3150 5250 3100
$Comp
L Device:R_Small_US R9
U 1 1 5CDFF68A
P 7100 3350
F 0 "R9" H 7168 3396 50  0000 L CNN
F 1 "10k" H 7168 3305 50  0000 L CNN
F 2 "Resistor_SMD:R_0603_1608Metric_Pad1.05x0.95mm_HandSolder" H 7100 3350 50  0001 C CNN
F 3 "~" H 7100 3350 50  0001 C CNN
	1    7100 3350
	1    0    0    -1  
$EndComp
Wire Wire Line
	7100 3500 7100 3450
Connection ~ 7100 3500
Wire Wire Line
	7100 3500 7200 3500
Wire Wire Line
	7100 3250 7100 3200
$Comp
L Device:R_Small_US R10
U 1 1 5CEAC4A3
P 7100 4050
F 0 "R10" H 7168 4096 50  0000 L CNN
F 1 "10k" H 7168 4005 50  0000 L CNN
F 2 "Resistor_SMD:R_0603_1608Metric_Pad1.05x0.95mm_HandSolder" H 7100 4050 50  0001 C CNN
F 3 "~" H 7100 4050 50  0001 C CNN
	1    7100 4050
	1    0    0    -1  
$EndComp
Wire Wire Line
	7100 3950 7100 3700
Connection ~ 7100 3700
Wire Wire Line
	7100 3700 7200 3700
Wire Wire Line
	7100 4250 7100 4150
$Comp
L Device:R_Small_US R12
U 1 1 5CEFD288
P 7450 4050
F 0 "R12" H 7518 4096 50  0000 L CNN
F 1 "10k" H 7518 4005 50  0000 L CNN
F 2 "Resistor_SMD:R_0603_1608Metric_Pad1.05x0.95mm_HandSolder" H 7450 4050 50  0001 C CNN
F 3 "~" H 7450 4050 50  0001 C CNN
	1    7450 4050
	1    0    0    -1  
$EndComp
Wire Wire Line
	7450 3950 7450 3900
Connection ~ 7450 3900
Wire Wire Line
	7450 3900 7650 3900
Wire Wire Line
	7450 4150 7450 4250
Wire Wire Line
	7100 4250 7450 4250
$Comp
L Device:R_Small_US R7
U 1 1 5CF71DF2
P 4800 3650
F 0 "R7" H 4868 3696 50  0000 L CNN
F 1 "10k" H 4868 3605 50  0000 L CNN
F 2 "Resistor_SMD:R_0603_1608Metric_Pad1.05x0.95mm_HandSolder" H 4800 3650 50  0001 C CNN
F 3 "~" H 4800 3650 50  0001 C CNN
	1    4800 3650
	-1   0    0    -1  
$EndComp
Wire Wire Line
	4800 3800 4800 3750
Connection ~ 4800 3800
Wire Wire Line
	4800 3800 5850 3800
$Comp
L power:VDD #PWR024
U 1 1 5CF832E7
P 4800 3450
F 0 "#PWR024" H 4800 3300 50  0001 C CNN
F 1 "VDD" H 4817 3623 50  0000 C CNN
F 2 "" H 4800 3450 50  0001 C CNN
F 3 "" H 4800 3450 50  0001 C CNN
	1    4800 3450
	1    0    0    -1  
$EndComp
Wire Wire Line
	4800 3550 4800 3500
$Comp
L power:+BATT #PWR038
U 1 1 5CF94EF5
P 7100 3200
F 0 "#PWR038" H 7100 3050 50  0001 C CNN
F 1 "+BATT" H 7115 3373 50  0000 C CNN
F 2 "" H 7100 3200 50  0001 C CNN
F 3 "" H 7100 3200 50  0001 C CNN
	1    7100 3200
	1    0    0    -1  
$EndComp
$Comp
L power:+BATT #PWR041
U 1 1 5CF9532A
P 7750 4200
F 0 "#PWR041" H 7750 4050 50  0001 C CNN
F 1 "+BATT" H 7765 4373 50  0000 C CNN
F 2 "" H 7750 4200 50  0001 C CNN
F 3 "" H 7750 4200 50  0001 C CNN
	1    7750 4200
	1    0    0    -1  
$EndComp
Wire Wire Line
	6650 3600 7200 3600
NoConn ~ 5850 4100
NoConn ~ 6650 4100
Wire Wire Line
	8250 900  8450 900 
Wire Wire Line
	8250 1000 8450 1000
$Comp
L power:PWR_FLAG #FLG02
U 1 1 5D0E6D26
P 2000 1700
F 0 "#FLG02" H 2000 1775 50  0001 C CNN
F 1 "PWR_FLAG" H 2000 1873 50  0000 C CNN
F 2 "" H 2000 1700 50  0001 C CNN
F 3 "~" H 2000 1700 50  0001 C CNN
	1    2000 1700
	1    0    0    -1  
$EndComp
Wire Wire Line
	1650 1800 2000 1800
Wire Wire Line
	2000 1800 2000 1700
Connection ~ 1650 1800
Wire Wire Line
	3150 4600 3150 4650
Wire Wire Line
	600  4250 600  4300
Wire Wire Line
	1050 4250 1150 4250
Wire Wire Line
	5450 5450 5550 5450
Wire Wire Line
	5450 5550 5550 5550
Wire Wire Line
	5450 5650 5550 5650
Wire Wire Line
	5450 5750 5550 5750
Wire Wire Line
	5450 5850 5550 5850
Wire Wire Line
	5450 6150 5550 6150
$Comp
L power:GND #PWR022
U 1 1 5CB128D4
P 4500 6050
F 0 "#PWR022" H 4500 5800 50  0001 C CNN
F 1 "GND" H 4505 5877 50  0000 C CNN
F 2 "" H 4500 6050 50  0001 C CNN
F 3 "" H 4500 6050 50  0001 C CNN
	1    4500 6050
	1    0    0    -1  
$EndComp
Wire Wire Line
	4550 6000 4500 6000
Wire Wire Line
	4500 6000 4500 6050
Text GLabel 4500 5450 0    39   Input ~ 0
I2C_SCL
Text GLabel 4500 5550 0    39   BiDi ~ 0
I2C_SDA
Text GLabel 3350 6300 2    39   Output ~ 0
I2C_SCL
Text GLabel 3350 6400 2    39   BiDi ~ 0
I2C_SDA
$Comp
L power:GND #PWR026
U 1 1 5CB86217
P 5000 7600
F 0 "#PWR026" H 5000 7350 50  0001 C CNN
F 1 "GND" H 5005 7427 50  0000 C CNN
F 2 "" H 5000 7600 50  0001 C CNN
F 3 "" H 5000 7600 50  0001 C CNN
	1    5000 7600
	1    0    0    -1  
$EndComp
Wire Wire Line
	5000 7600 5000 7350
Wire Wire Line
	4500 5450 4550 5450
Wire Wire Line
	4550 5550 4500 5550
Text GLabel 4500 5700 0    39   Output ~ 0
INPUT_INT
Text GLabel 1200 4900 0    39   Input ~ 0
INPUT_INT
Wire Wire Line
	4500 5700 4550 5700
$Comp
L power:VDD #PWR025
U 1 1 5CBE9DD2
P 5000 4450
F 0 "#PWR025" H 5000 4300 50  0001 C CNN
F 1 "VDD" H 5017 4623 50  0000 C CNN
F 2 "" H 5000 4450 50  0001 C CNN
F 3 "" H 5000 4450 50  0001 C CNN
	1    5000 4450
	1    0    0    -1  
$EndComp
Wire Wire Line
	5000 4450 5000 4500
$Comp
L power:VDD #PWR017
U 1 1 5CBFE713
P 4050 5800
F 0 "#PWR017" H 4050 5650 50  0001 C CNN
F 1 "VDD" H 4067 5973 50  0000 C CNN
F 2 "" H 4050 5800 50  0001 C CNN
F 3 "" H 4050 5800 50  0001 C CNN
	1    4050 5800
	1    0    0    -1  
$EndComp
Wire Wire Line
	4050 5800 4050 5850
Wire Wire Line
	4050 5850 4550 5850
Wire Wire Line
	5450 5950 5550 5950
Wire Wire Line
	5550 6050 5450 6050
$Comp
L Device:C_Small C4
U 1 1 5CFB466A
P 5400 4650
F 0 "C4" H 5492 4696 50  0000 L CNN
F 1 "0.1uF" H 5492 4605 50  0000 L CNN
F 2 "Capacitor_SMD:C_0603_1608Metric_Pad1.05x0.95mm_HandSolder" H 5400 4650 50  0001 C CNN
F 3 "~" H 5400 4650 50  0001 C CNN
	1    5400 4650
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR029
U 1 1 5CFB4E3D
P 5400 4800
F 0 "#PWR029" H 5400 4550 50  0001 C CNN
F 1 "GND" H 5405 4627 50  0000 C CNN
F 2 "" H 5400 4800 50  0001 C CNN
F 3 "" H 5400 4800 50  0001 C CNN
	1    5400 4800
	1    0    0    -1  
$EndComp
Wire Wire Line
	5400 4800 5400 4750
Wire Wire Line
	5400 4550 5400 4500
Wire Wire Line
	5400 4500 5000 4500
Connection ~ 5000 4500
Wire Wire Line
	5000 4500 5000 4850
$Comp
L Device:C_Small C5
U 1 1 5D028A7A
P 5700 2800
F 0 "C5" H 5792 2846 50  0000 L CNN
F 1 "0.1uF" H 5792 2755 50  0000 L CNN
F 2 "Capacitor_SMD:C_0603_1608Metric_Pad1.05x0.95mm_HandSolder" H 5700 2800 50  0001 C CNN
F 3 "~" H 5700 2800 50  0001 C CNN
	1    5700 2800
	-1   0    0    -1  
$EndComp
$Comp
L power:GND #PWR030
U 1 1 5D0DE4F7
P 5700 2950
F 0 "#PWR030" H 5700 2700 50  0001 C CNN
F 1 "GND" H 5705 2777 50  0000 C CNN
F 2 "" H 5700 2950 50  0001 C CNN
F 3 "" H 5700 2950 50  0001 C CNN
	1    5700 2950
	1    0    0    -1  
$EndComp
Wire Wire Line
	5700 2950 5700 2900
Wire Wire Line
	5700 2700 5700 2650
Wire Wire Line
	5700 2650 5800 2650
Connection ~ 5800 2650
Wire Wire Line
	5800 2650 5800 2600
$Comp
L Device:C_Small C8
U 1 1 5D11AD0A
P 6800 2800
F 0 "C8" H 6892 2846 50  0000 L CNN
F 1 "0.1uF" H 6892 2755 50  0000 L CNN
F 2 "Capacitor_SMD:C_0603_1608Metric_Pad1.05x0.95mm_HandSolder" H 6800 2800 50  0001 C CNN
F 3 "~" H 6800 2800 50  0001 C CNN
	1    6800 2800
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR037
U 1 1 5D11B308
P 6800 2950
F 0 "#PWR037" H 6800 2700 50  0001 C CNN
F 1 "GND" H 6805 2777 50  0000 C CNN
F 2 "" H 6800 2950 50  0001 C CNN
F 3 "" H 6800 2950 50  0001 C CNN
	1    6800 2950
	1    0    0    -1  
$EndComp
Wire Wire Line
	6800 2950 6800 2900
Wire Wire Line
	6800 2700 6800 2650
Wire Wire Line
	6800 2650 6700 2650
Connection ~ 6700 2650
Wire Wire Line
	6700 2650 6700 2600
$Comp
L power:GND #PWR043
U 1 1 5D37F07D
P 7950 4700
F 0 "#PWR043" H 7950 4450 50  0001 C CNN
F 1 "GND" H 7955 4527 50  0000 C CNN
F 2 "" H 7950 4700 50  0001 C CNN
F 3 "" H 7950 4700 50  0001 C CNN
	1    7950 4700
	1    0    0    -1  
$EndComp
Wire Wire Line
	7950 4700 7950 4650
Wire Wire Line
	8400 4400 8400 4900
Text GLabel 3900 4850 0    39   Output ~ 0
JTAG_MTDO
Text GLabel 3900 4650 0    39   Input ~ 0
JTAG_MTCK
Text GLabel 3900 4750 0    39   Input ~ 0
JTAG_MTMS
Text GLabel 3900 4550 0    39   Input ~ 0
JTAG_MTDI
Wire Wire Line
	3900 4850 3950 4850
Wire Wire Line
	3950 4750 3900 4750
Wire Wire Line
	3900 4650 3950 4650
Wire Wire Line
	3900 4550 3950 4550
$Comp
L Connector_Generic:Conn_01x06 J2
U 1 1 5D4CA230
P 4150 4650
F 0 "J2" H 4450 4700 50  0000 C CNN
F 1 "Conn_01x06" H 4450 4600 50  0000 C CNN
F 2 "Connector:Tag-Connect_TC2030-IDC-NL_2x03_P1.27mm_Vertical" H 4150 4650 50  0001 C CNN
F 3 "~" H 4150 4650 50  0001 C CNN
	1    4150 4650
	1    0    0    -1  
$EndComp
Text GLabel 5550 6700 2    39   Output ~ 0
LCD_RESET
Text GLabel 5550 6800 2    39   Output ~ 0
LCD_BL
Text GLabel 1200 4800 0    39   Input ~ 0
LCD_TE
Wire Wire Line
	1200 4800 1250 4800
Wire Wire Line
	1200 4900 1250 4900
$Comp
L power:PWR_FLAG #FLG03
U 1 1 5D70F89F
P 4600 1100
F 0 "#FLG03" H 4600 1175 50  0001 C CNN
F 1 "PWR_FLAG" H 4600 1273 50  0000 C CNN
F 2 "" H 4600 1100 50  0001 C CNN
F 3 "~" H 4600 1100 50  0001 C CNN
	1    4600 1100
	1    0    0    -1  
$EndComp
Wire Wire Line
	4600 1100 4600 1150
Wire Wire Line
	4600 1150 4300 1150
Connection ~ 4300 1150
Wire Wire Line
	8850 5500 8800 5500
$Comp
L power:GND #PWR047
U 1 1 5CA27776
P 8850 6500
F 0 "#PWR047" H 8850 6250 50  0001 C CNN
F 1 "GND" H 8855 6327 50  0000 C CNN
F 2 "" H 8850 6500 50  0001 C CNN
F 3 "" H 8850 6500 50  0001 C CNN
	1    8850 6500
	1    0    0    -1  
$EndComp
Connection ~ 8850 6450
Connection ~ 8850 6200
Wire Wire Line
	8850 6200 8850 6450
$Comp
L power:GND #PWR050
U 1 1 5D7A48D8
P 9050 5850
F 0 "#PWR050" H 9050 5600 50  0001 C CNN
F 1 "GND" H 9055 5677 50  0000 C CNN
F 2 "" H 9050 5850 50  0001 C CNN
F 3 "" H 9050 5850 50  0001 C CNN
	1    9050 5850
	1    0    0    -1  
$EndComp
Wire Wire Line
	10150 4150 10150 4000
Wire Wire Line
	10150 4550 10150 5250
Wire Wire Line
	10150 5250 8800 5250
Text GLabel 8900 4750 0    39   Input ~ 0
LCD_BL
Wire Wire Line
	8300 4400 8400 4400
Connection ~ 8300 4400
$Comp
L Device:Q_PNP_BEC Q2
U 1 1 5D7BBC33
P 10050 4350
F 0 "Q2" H 10240 4304 50  0000 L CNN
F 1 "S9015" H 10240 4395 50  0000 L CNN
F 2 "Package_TO_SOT_SMD:SOT-23_Handsoldering" H 10250 4450 50  0001 C CNN
F 3 "~" H 10050 4350 50  0001 C CNN
	1    10050 4350
	1    0    0    1   
$EndComp
$Comp
L Device:R_Small_US R15
U 1 1 5D8F0B46
P 9700 4350
F 0 "R15" V 9800 4350 50  0000 C CNN
F 1 "1k" V 9900 4350 50  0000 C CNN
F 2 "Resistor_SMD:R_0603_1608Metric_Pad1.05x0.95mm_HandSolder" H 9700 4350 50  0001 C CNN
F 3 "~" H 9700 4350 50  0001 C CNN
	1    9700 4350
	0    1    1    0   
$EndComp
Wire Wire Line
	9800 4350 9850 4350
Wire Wire Line
	9050 5350 9050 5400
Wire Wire Line
	9050 5850 9050 5800
$Comp
L phone:NSI45015WT1G U7
U 1 1 5DB5BF49
P 9050 5600
F 0 "U7" H 9153 5646 50  0000 L CNN
F 1 "NSI45015WT1G" H 9153 5555 50  0000 L CNN
F 2 "Diode_SMD:D_SOD-123" H 9050 5600 50  0001 C CNN
F 3 "https://datasheet.lcsc.com/szlcsc/ON-Semicon-ON-NSI45015WT1G_C328096.pdf" H 9050 5600 50  0001 C CNN
	1    9050 5600
	1    0    0    -1  
$EndComp
$Comp
L Switch:SW_Push SW1
U 1 1 5C9A9A89
P 850 4250
F 0 "SW1" H 850 4575 50  0000 C CNN
F 1 "SW_Push" H 850 4484 50  0000 C CNN
F 2 "Button_Switch_SMD:SW_SPST_EVQPE1" H 850 4250 50  0001 C CNN
F 3 "~" H 850 4250 50  0001 C CNN
	1    850  4250
	-1   0    0    -1  
$EndComp
$Comp
L power:GND #PWR016
U 1 1 5DC8B84C
P 3900 5000
F 0 "#PWR016" H 3900 4750 50  0001 C CNN
F 1 "GND" H 3905 4827 50  0000 C CNN
F 2 "" H 3900 5000 50  0001 C CNN
F 3 "" H 3900 5000 50  0001 C CNN
	1    3900 5000
	1    0    0    -1  
$EndComp
Wire Wire Line
	3950 4950 3900 4950
Wire Wire Line
	3900 4950 3900 5000
$Comp
L power:VDD #PWR015
U 1 1 5DCA34FE
P 3900 4400
F 0 "#PWR015" H 3900 4250 50  0001 C CNN
F 1 "VDD" H 3917 4573 50  0000 C CNN
F 2 "" H 3900 4400 50  0001 C CNN
F 3 "" H 3900 4400 50  0001 C CNN
	1    3900 4400
	1    0    0    -1  
$EndComp
Wire Wire Line
	3950 4450 3900 4450
Wire Wire Line
	3900 4450 3900 4400
$Comp
L Device:R_Small_US R4
U 1 1 5DDBAE04
P 2950 6150
F 0 "R4" H 3018 6196 50  0000 L CNN
F 1 "10k" H 3018 6105 50  0000 L CNN
F 2 "Resistor_SMD:R_0603_1608Metric_Pad1.05x0.95mm_HandSolder" H 2950 6150 50  0001 C CNN
F 3 "~" H 2950 6150 50  0001 C CNN
	1    2950 6150
	1    0    0    -1  
$EndComp
Wire Wire Line
	2950 6300 2950 6250
Connection ~ 2950 6300
Wire Wire Line
	2950 6300 3350 6300
$Comp
L power:VDD #PWR012
U 1 1 5DE02908
P 3100 5950
F 0 "#PWR012" H 3100 5800 50  0001 C CNN
F 1 "VDD" H 3117 6123 50  0000 C CNN
F 2 "" H 3100 5950 50  0001 C CNN
F 3 "" H 3100 5950 50  0001 C CNN
	1    3100 5950
	1    0    0    -1  
$EndComp
Wire Wire Line
	2950 6050 2950 6000
$Comp
L Device:R_Small_US R5
U 1 1 5DE1B8D1
P 3250 6150
F 0 "R5" H 3318 6196 50  0000 L CNN
F 1 "10k" H 3318 6105 50  0000 L CNN
F 2 "Resistor_SMD:R_0603_1608Metric_Pad1.05x0.95mm_HandSolder" H 3250 6150 50  0001 C CNN
F 3 "~" H 3250 6150 50  0001 C CNN
	1    3250 6150
	1    0    0    -1  
$EndComp
Wire Wire Line
	3250 6400 3250 6250
Connection ~ 3250 6400
Wire Wire Line
	3250 6400 2450 6400
Wire Wire Line
	2950 6000 3100 6000
Wire Wire Line
	3250 6000 3250 6050
Wire Wire Line
	3100 6000 3100 5950
Connection ~ 3100 6000
Wire Wire Line
	3100 6000 3250 6000
Wire Wire Line
	6300 1550 6800 1550
Wire Wire Line
	6800 1550 6800 1500
Connection ~ 6300 1550
Wire Wire Line
	6300 1250 6800 1250
Wire Wire Line
	6800 1250 6800 1300
Connection ~ 6300 1250
Wire Wire Line
	6300 1250 6300 1000
$Comp
L Device:CP_Small C?
U 1 1 5CE0C170
P 6800 1400
AR Path="/5C9AE52D/5CE0C170" Ref="C?"  Part="1" 
AR Path="/5CE0C170" Ref="C7"  Part="1" 
F 0 "C7" H 6888 1446 50  0000 L CNN
F 1 "100uF" H 6888 1355 50  0000 L CNN
F 2 "Capacitor_Tantalum_SMD:CP_EIA-3528-15_AVX-H_Pad1.50x2.35mm_HandSolder" H 6800 1400 50  0001 C CNN
F 3 "~" H 6800 1400 50  0001 C CNN
	1    6800 1400
	1    0    0    -1  
$EndComp
$Comp
L power:VDD #PWR023
U 1 1 5CE29625
P 4600 1800
F 0 "#PWR023" H 4600 1650 50  0001 C CNN
F 1 "VDD" H 4617 1973 50  0000 C CNN
F 2 "" H 4600 1800 50  0001 C CNN
F 3 "" H 4600 1800 50  0001 C CNN
	1    4600 1800
	1    0    0    -1  
$EndComp
Wire Wire Line
	4150 1950 4600 1950
Wire Wire Line
	4600 1950 4600 1800
$Comp
L Device:Q_NPN_BEC Q1
U 1 1 5CF17E4A
P 9400 4750
F 0 "Q1" H 9590 4704 50  0000 L CNN
F 1 "BC847BLT3G" H 9590 4795 50  0000 L CNN
F 2 "Package_TO_SOT_SMD:SOT-23_Handsoldering" H 9600 4850 50  0001 C CNN
F 3 "~" H 9400 4750 50  0001 C CNN
	1    9400 4750
	1    0    0    -1  
$EndComp
Wire Wire Line
	7950 4450 7950 4400
Wire Wire Line
	7950 4400 8300 4400
$Comp
L Device:R_Small_US R14
U 1 1 5D1551B2
P 9050 4750
F 0 "R14" V 9150 4750 50  0000 C CNN
F 1 "1k" V 9250 4750 50  0000 C CNN
F 2 "Resistor_SMD:R_0603_1608Metric_Pad1.05x0.95mm_HandSolder" H 9050 4750 50  0001 C CNN
F 3 "~" H 9050 4750 50  0001 C CNN
	1    9050 4750
	0    1    1    0   
$EndComp
Wire Wire Line
	9200 4750 9150 4750
Wire Wire Line
	8900 4750 8950 4750
Wire Wire Line
	9500 4550 9500 4350
Wire Wire Line
	9500 4350 9600 4350
$Comp
L power:GND #PWR051
U 1 1 5D260496
P 9500 5000
F 0 "#PWR051" H 9500 4750 50  0001 C CNN
F 1 "GND" H 9505 4827 50  0000 C CNN
F 2 "" H 9500 5000 50  0001 C CNN
F 3 "" H 9500 5000 50  0001 C CNN
	1    9500 5000
	1    0    0    -1  
$EndComp
Wire Wire Line
	9500 5000 9500 4950
$Comp
L power:+BATT #PWR053
U 1 1 5D27E36B
P 10150 4000
F 0 "#PWR053" H 10150 3850 50  0001 C CNN
F 1 "+BATT" H 10165 4173 50  0000 C CNN
F 2 "" H 10150 4000 50  0001 C CNN
F 3 "" H 10150 4000 50  0001 C CNN
	1    10150 4000
	1    0    0    -1  
$EndComp
Text GLabel 4300 2150 2    39   Output ~ 0
BATT_~STDBY
Wire Wire Line
	4150 2150 4300 2150
Text GLabel 5550 6150 2    39   Input ~ 0
BATT_~CHRG
Text GLabel 5550 6300 2    39   Input ~ 0
BATT_~STDBY
$Comp
L phone:PCAL6416A U2
U 1 1 5D4F7AB8
P 5000 6200
F 0 "U2" H 5000 4961 50  0000 C CNN
F 1 "PCAL6416A" H 5000 4870 50  0000 C CNN
F 2 "Package_SO:TSSOP-24_4.4x7.8mm_P0.65mm" H 5000 6200 50  0001 C CNN
F 3 "https://static6.arrow.com/aropdfconversion/ada1d8a6c7509095185a3a33352339d459d491a2/2688029915511161pcal6416a.pdf" H 5000 6200 50  0001 C CNN
	1    5000 6200
	1    0    0    -1  
$EndComp
Wire Wire Line
	4950 4950 4950 4850
Wire Wire Line
	4950 4850 5000 4850
Wire Wire Line
	5050 4950 5050 4850
Wire Wire Line
	5050 4850 5000 4850
Connection ~ 5000 4850
Wire Wire Line
	5450 6300 5550 6300
NoConn ~ 9600 2750
Text GLabel 2550 5900 2    39   Output ~ 0
LCD_~CS
Wire Wire Line
	5450 6800 5550 6800
Wire Wire Line
	5550 6900 5450 6900
Wire Wire Line
	5450 7000 5550 7000
Text GLabel 5450 3600 0    39   Input ~ 0
GPS_RESET_2.8
Text GLabel 5550 6900 2    39   Output ~ 0
GPS_RESET_2.8
Text GLabel 5450 3700 0    39   Input ~ 0
GSM_PWRKEY_2.8
Text GLabel 5550 7000 2    39   Output ~ 0
GSM_PWRKEY_2.8
Wire Wire Line
	5450 6700 5550 6700
Text GLabel 2550 6500 2    39   Output ~ 0
PCM_CLK
Text GLabel 2550 5000 2    39   Output ~ 0
PCM_SYNC
Text GLabel 2550 6600 2    39   Output ~ 0
PCM_IN
Text GLabel 2550 6700 2    39   Input ~ 0
PCM_OUT
Wire Wire Line
	5450 6400 6050 6400
$Comp
L Switch:SW_Push SW5
U 1 1 5D9343D0
P 6650 6500
F 0 "SW5" H 6650 6825 50  0000 C CNN
F 1 "VolUp" H 6650 6734 50  0000 C CNN
F 2 "Button_Switch_THT:SW_Tactile_SPST_Angled_PTS645Vx58-2LFS" H 6650 6500 50  0001 C CNN
F 3 "~" H 6650 6500 50  0001 C CNN
	1    6650 6500
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR040
U 1 1 5D950C6B
P 7500 6550
F 0 "#PWR040" H 7500 6300 50  0001 C CNN
F 1 "GND" H 7505 6377 50  0000 C CNN
F 2 "" H 7500 6550 50  0001 C CNN
F 3 "" H 7500 6550 50  0001 C CNN
	1    7500 6550
	1    0    0    -1  
$EndComp
Wire Wire Line
	5450 6500 6450 6500
$Comp
L Switch:SW_Push SW6
U 1 1 5D9897BE
P 7100 6600
F 0 "SW6" H 7100 6925 50  0000 C CNN
F 1 "VolDown" H 7100 6834 50  0000 C CNN
F 2 "Button_Switch_THT:SW_Tactile_SPST_Angled_PTS645Vx58-2LFS" H 7100 6600 50  0001 C CNN
F 3 "~" H 7100 6600 50  0001 C CNN
	1    7100 6600
	1    0    0    -1  
$EndComp
Wire Wire Line
	5450 6600 6900 6600
Wire Wire Line
	6450 6400 7350 6400
Wire Wire Line
	7350 6400 7350 6500
Wire Wire Line
	7350 6600 7300 6600
Wire Wire Line
	6850 6500 7350 6500
Connection ~ 7350 6500
Wire Wire Line
	7350 6500 7350 6600
Wire Wire Line
	7500 6550 7500 6500
Wire Wire Line
	7500 6500 7350 6500
$Comp
L Switch:SW_Push SW4
U 1 1 5D917463
P 6250 6400
F 0 "SW4" H 6250 6725 50  0000 C CNN
F 1 "Lock" H 6250 6634 50  0000 C CNN
F 2 "Button_Switch_THT:SW_Tactile_SPST_Angled_PTS645Vx58-2LFS" H 6250 6400 50  0001 C CNN
F 3 "~" H 6250 6400 50  0001 C CNN
	1    6250 6400
	1    0    0    -1  
$EndComp
Text GLabel 9700 1300 2    39   Output ~ 0
GSM_RI
Wire Wire Line
	9600 1300 9700 1300
Text GLabel 6700 4000 2    39   Input ~ 0
GSM_RI
Wire Wire Line
	7450 4250 7750 4250
Wire Wire Line
	7750 4250 7750 4200
Connection ~ 7450 4250
Wire Wire Line
	6650 4000 6700 4000
Text GLabel 4200 4000 0    39   Output ~ 0
GSM_RI_2.8
Wire Wire Line
	5850 4000 4350 4000
$Comp
L Device:R_Small_US R6
U 1 1 5DC1E4EB
P 4350 3750
F 0 "R6" H 4418 3796 50  0000 L CNN
F 1 "10k" H 4418 3705 50  0000 L CNN
F 2 "Resistor_SMD:R_0603_1608Metric_Pad1.05x0.95mm_HandSolder" H 4350 3750 50  0001 C CNN
F 3 "~" H 4350 3750 50  0001 C CNN
	1    4350 3750
	-1   0    0    -1  
$EndComp
Wire Wire Line
	4350 4000 4350 3850
Connection ~ 4350 4000
Wire Wire Line
	4350 4000 4200 4000
Wire Wire Line
	4800 3500 4350 3500
Wire Wire Line
	4350 3500 4350 3650
Connection ~ 4800 3500
Wire Wire Line
	4800 3500 4800 3450
Text GLabel 2550 4800 2    39   Input ~ 0
GSM_RI_2.8
$Comp
L Connector:Conn_01x01_Female J5
U 1 1 5DE9CCC6
P 2100 900
F 0 "J5" H 1992 675 50  0000 C CNN
F 1 "BATT+" H 1992 766 50  0000 C CNN
F 2 "Connector_PinSocket_2.54mm:PinSocket_1x01_P2.54mm_Vertical" H 2100 900 50  0001 C CNN
F 3 "~" H 2100 900 50  0001 C CNN
	1    2100 900 
	-1   0    0    1   
$EndComp
$Comp
L Connector:Conn_01x01_Female J6
U 1 1 5DF18DEB
P 2100 1300
F 0 "J6" H 1992 1075 50  0000 C CNN
F 1 "BATT-" H 1992 1166 50  0000 C CNN
F 2 "Connector_PinSocket_2.54mm:PinSocket_1x01_P2.54mm_Vertical" H 2100 1300 50  0001 C CNN
F 3 "~" H 2100 1300 50  0001 C CNN
	1    2100 1300
	-1   0    0    1   
$EndComp
Wire Wire Line
	2300 1300 2700 1300
$Comp
L power:PWR_FLAG #FLG0102
U 1 1 5DF195A7
P 2700 1200
F 0 "#FLG0102" H 2700 1275 50  0001 C CNN
F 1 "PWR_FLAG" H 2700 1373 50  0000 C CNN
F 2 "" H 2700 1200 50  0001 C CNN
F 3 "~" H 2700 1200 50  0001 C CNN
	1    2700 1200
	1    0    0    -1  
$EndComp
Wire Wire Line
	2700 1200 2700 1300
Connection ~ 2700 1300
Wire Wire Line
	2700 1300 2900 1300
Wire Wire Line
	2300 900  2900 900 
Wire Wire Line
	8450 1150 8250 1150
Wire Wire Line
	8450 1250 8250 1250
NoConn ~ 8250 900 
NoConn ~ 8250 1000
NoConn ~ 8250 1150
NoConn ~ 8250 1250
$EndSCHEMATC
